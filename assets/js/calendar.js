...
window.onload = () => {
    // Start calendar
    const cal = new Calendar('calendar1');
    ...
    const HEUSS = new Calendar('calendar2');
    HEUSS.displayCurrentMonth();
    // Bind next and previous buttons
    ...
    // Attach event listner to the Change Week start day radio buttons
    const Heussradios = document.getElementsByName('changeWeekStart');
    Heussradios.forEach(radio => {
        radio.addEventListener('change', () => {
            HEUSS.changeStarterWeekDay(radio.value);
        });
    });
};
